...
class TableBackup(BackupManager):
    ...
    def _backup(
        ...
        backup_disks: Optional[ClickHouseBackupDisks] = None,
    ) -> None:
        """
    ...
    def _copy_cloud_storage_data(
        ...
        table: Table,
        disk: Disk,
        backup_disks: Optional[ClickHouseBackupDisks],
    ) -> Tuple[Table, Disk, Optional[Disk]]:
        """
        Copy data of a table on a cloud storage disk into the backup.

        Returns what uploading of the metadata needs: the table, its disk and
        the temporary disk holding metadata of the copies. The copy is checked
        explicitly, since clickhouse-disks reports its errors with a zero exit
        code.
        """
        if not backup_disks:
    ...
    def _backup_frozen_table_data(
        ...
        def deduplicate_cloud_parts_in_batch(
            ...
            for part_name, frozen_part in frozen_parts.items():
                part = deduplicated_parts.get(part_name)
                if part and part.disk_name == frozen_part.disk_name:
                    context.ch_ctl.remove_freezed_part(frozen_part)
                else:
                    part = PartMetadata.from_frozen_part(
        ...
        deduplicate_cloud_parts = context.backup_meta.cloud_storage.data_copied
        ...
        for data_path, disk in table.paths_with_disks:
            for fpart in context.ch_ctl.scan_frozen_parts(
                ...
                if disk.type == "s3":
                    if disk.cache_path or not deduplicate_cloud_parts:
